...
class SRS_Operator_Module(nn.Module):
    def __init__(self,
                 scale_factor,
                 encode_channels,
                 fc_channels,
                 size,
                 groups=1):
        super(SRS_Operator_Module, self).__init__()
...
class SRS_Operator(nn.Module):
    def __init__(self,
                 ...
                 version=0,
                 groups=1):
        super(SRS_Operator, self).__init__()
        ...
        # Calculate the number of channels after downsampling
        ...
        self.relu = nn.ReLU(inplace=True)   # Use ConvSNP model to activate features, then restructure
        self.sample_encode_conv = nn.Conv2d(self.in_channels, self.encode_channels, kernel_size=1, stride=1, padding=0)
        self.sample = SRS_Operator_Module(self.scale_factor, self.encode_channels, self.fc_channels,
    ...
    def forward(self, x):
        ...
        x = self.relu(x)   # Use ConvSNP model to activate features, then restructure
...
# Test SRS_Operator module
# # Create input tensor
# input_tensor = torch.randn(32, 240, 240)
# # Downsampling
# SRS_Operator_downsample = SRS_Operator(0.5, 32, 64)
# output_tensor = SRS_Operator_downsample(input_tensor.unsqueeze(0)).squeeze(0)
# print("Downsampled output shape:", output_tensor.shape)
# # Upsampling
# SRS_Operator_upsample = SRS_Operator(2, 64, 32)
# output_tensor = SRS_Operator_upsample(output_tensor.unsqueeze(0)).squeeze(0)
# print("Upsampled output shape:", output_tensor.shape)
