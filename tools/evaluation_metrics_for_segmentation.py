...
from tools.file_management import get_filenames, save_csv_mean_segmentation_performance, save_csv_segmentation_table
...
def dice_coefficient_1(binary_segmentation, binary_gt_label,binary_segmentation_not,binary_gt_label_not):
    ...
    if segmentation_pixels == 0 or intersection == 0:
        Pre = 0
    else:
...
#单独分割视盘
def evaluate_binary_segmentation_OD(segmentation, gt_label):

    # compute the Dice coefficient for the optic disc
    disc_dice,disc_jac,disc_acc,disc_sen,disc_pre = dice_coefficient_1(segmentation==0, gt_label==0,segmentation>0,gt_label>0)
    # compute the absolute error between the cup to disc ratio estimated from the segmentation vs. the gt label
    cdr = absolute_error(vertical_cup_to_disc_ratio(segmentation), vertical_cup_to_disc_ratio(gt_label))

    return disc_dice,disc_jac,disc_acc,disc_sen,disc_pre,cdr

#单独分割视杯
def evaluate_binary_segmentation_OC(segmentation, gt_label):

    cup_dice,cup_jac,cup_acc,cup_sen,cup_pre = dice_coefficient_1(segmentation==0, gt_label==0,segmentation>0,gt_label>0)
    cdr = absolute_error(vertical_cup_to_disc_ratio(segmentation), vertical_cup_to_disc_ratio(gt_label))

    return cup_dice,cup_jac,cup_acc,cup_sen,cup_pre,cdr
